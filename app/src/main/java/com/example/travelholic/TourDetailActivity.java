...
public class TourDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnApply.setOnClickListener(v -> {
            if (btnApply.getText().toString().equals("Apply")) {
                String applyUrl = "http://10.0.2.2/travelholic-app/server/index.php?controller=notification&action=apply&tour_id="
            ...
            public void onResponse(@NotNull Call call, @NotNull Response response) {
                TourDetailActivity.this.runOnUiThread(() -> {
                    try {
                        ...
                        urlImage = "http://10.0.2.2/travelholic-app/server/" + jsonObject.getString("image");
    ...
    private void refreshComments() {
        String url = "http://10.0.2.2/travelholic-app/server/index.php?controller=comment&action=load_all&tour_id=" + tourId;
        Request request = new Request.Builder()
                .url(url)
                .build();
        OkHttpClient client = new OkHttpClient();
